...
const termsPath = get("--terms");
if (!profilePath || !inputPath) {
  console.error("Usage: node scripts/validate-caption-pages.mjs --profile <profile.json> --input <captions.json|read_captions.txt> [--terms <terminology.json>]");
...
function deepMerge(base, overlay) {
  ...
  for (const [key, value] of Object.entries(overlay)) merged[key] = deepMerge(merged[key], value);
  return merged;
}

function loadProfile(file, seen = new Set()) {
  const absolute = path.resolve(file);
  if (seen.has(absolute)) throw new Error(`profile inheritance cycle: ${[...seen, absolute].join(" -> ")}`);
  seen.add(absolute);
  const overlay = readJson(absolute);
  if (!overlay.extends) return overlay;
  const parent = loadProfile(path.resolve(path.dirname(absolute), overlay.extends), seen);
  const {extends: _extends, ...resolvedOverlay} = overlay;
  return deepMerge(parent, resolvedOverlay);
}
...
try {
  ...
  console.error(`FAIL invalid profile: ${error.message}`);
  process.exit(1);
}
...
function loadTerminology() {
  if (termsPath) {
    ...
    try {
      ...
      errors.push(`cannot load --terms file ${file}: ${error.message}`);
      return {};
    }
  }
