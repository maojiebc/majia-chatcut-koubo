...
- **Rule Registry foundation — SHIPPED**：18 条规则覆盖六个治理域，具备 stable ID、kind、severity、来源、`deny`/`tighten-only` 覆盖语义、waiver/fallback、runtime/registry-contract 执行级别和逐规则 pass/fail fixture；其中 4 条新增视觉决策规则治理证据优先、自动选择门槛、反模式与单一主任务；
...
- 未批准、拒绝、撤销、部分 scope、plan/style/timeline 变化均关闭执行门；匿名批准 fixture、路径边界和失效回归已进入 `npm run verify`；
- **Explainable planner foundation — SHIPPED**：从 transcript/edit IR 确定性计算总词数、保留词、首段密度、证据覆盖、低置信/内容风险词和 remove/reorder 数量，每个 signal 都携带 basis refs；
- **Visual Decision Contract v0 — SHIPPED**：视觉候选使用六维透明评分并绑定来源/版权证据、反模式、人工批准与回退；低于门槛可进入人工复核，但不会被标记为可执行；
- Hook → Problem → Contrast/Example → Method → Result → SoftCTA 仅作为 positional candidate 引用现有 segment/word，`inventedContent=false` 且必须人工语义审核；低置信、风险词和破坏性编辑进入 decision queue。planner 明确不输出生成文案或“爆款概率”，revision/fixture 漂移 fail closed。
...
- **Capability profile + live route gate — SHIPPED**：profile 明确绑定 host build、tool schema hash、observedAt/expiresAt、五类 mandatory capability、probe evidence、非阻断 content-free canary 与逐能力 fallback；
- 缺少 current canary 时审计可证明地保持 `unverified` 并路由到 fake/manual/blocked；过期、canary 失败或 evidence 不全时，任何 live route 请求 fail closed。当前匿名 fixture 故意保持 `not-run`，不宣称已连接 ChatCut；真实 adapter 与 live canary 仍需宿主工具面和当前 build 证据。
...
- export authorization 必须为 approved，并精确绑定最终 artifact hash 与 plan hash。匿名 probe/report fixture 已进入 `npm run verify`；当前 Rule Registry 18 条规则全部具备 runtime gate；
...
### R6 · 反馈与规则治理 — IN PROGRESS

目标：建立可复盘闭环，同时防止一次高播放或单一样本污染生产规则。

当前交付：

- **Feedback governance foundation — SHIPPED**：privacy-safe event Schema 仅接受匿名 project/run/event hash、稳定 failure signature、规则引用和 allowlisted 数值/布尔指标；additional properties 与内容字段/绝对路径双层阻断，默认 external sharing 为 `not-authorized`；
- suggested update queue 只生成 `suggested` 候选，不写 registry/profile；进入 review 必须达到重复样本门槛，批准/发布必须有证据、反例、owner、reviewer 与回滚记录；
- hard policy 发布还需显式的人工作业记录与语义版本升级；`autoApply=false` 和 `human-reviewed-release` 为 Schema 常量。匿名 fixture 与 CLI 已进入 `npm run verify`，当前不外发诊断包，也不连接线上反馈源。
...
| 5 | Explainable planner foundation — SHIPPED | scorecard、叙事状态建议、risk/decision queue |
...
| 11 | Feedback governance foundation — SHIPPED | privacy-safe event schema、suggested update queue |
