...
- **分页引擎默认按长度预算硬切**（break=length），会拆词断短语。按气口分卡的机器路径：
  1. `read_captions {json:{words:true}}` 拿词级 key（全片输出可能超限——落文件后 jq/python 提取；`frame` 参数实测无效，始终返回全片）。
  2. `edit_captions {action:"display_text", json:{overrides:[{key, forcePageBreak:true}]}}` 在气口词上打强制分页。
  3. **forced break 之间的片段控制在 ≤22 显示字**（以你 profile 的每行上限为准），引擎就不再插 length 切。
  4. `{key, hidden:true}` 隐藏 ASR 多转词；`{key, text}` 做词文本覆盖（低自由度规则见 `captions-terminology.md`）。
- **源 item 边界（sourceStart 跳变处）引擎自动强分页**：跨该边界的句子无法合并成一页。`split_item` 出来的连续两段不受此限（源连续）。
