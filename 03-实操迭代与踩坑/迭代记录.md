...
## 2026-07-25 · AI Hero 终验导出 E3 验收回执
...
- cases_read:
  - `FR-2026-07-24-AI-HERO-001`
- adopted:
  - 最终导出文件与旧文件先做 artifact identity 对账，再开始 QA。
  - 完整解码、全片每 5 秒抽帧、黑帧 / 冻结 / scene 扫描和定点逐帧并行使用。
  - 冻结检测只产生候选；录屏静止页必须扩大窗口解释，不能直接判坏帧。
  - 音频同时检查 integrated loudness、LRA、true peak、NaN / Inf、短窗峰值与 sample difference。
  - 视觉通过与音频发布门禁分开报告，不用整体“通过”掩盖单项失败。
- rejected_or_deferred:
  - 不把片尾约 0.48 秒的有意黑场误报为正文黑屏。
  - 不因最终真峰值没有达到数字 0 就宣称音频安全；约 `-0.02 dBFS` 仍缺少发布余量。
  - 不简单提高整体增益；当前同时存在整体偏轻和孤立近零峰值。
  - ChatCut 当前只暴露逐 item 增益、淡化和 crossfade，不能把它们冒充母线级 limiter / loudness normalization。
  - 不用 FFmpeg 改写或覆盖用户的 ChatCut 导出。
  - 本轮只累积 field report，不升级版本、不创建 GitHub Release、不同步 ClawHub。
- new_canaries:
  - `overall-quiet-single-near-zero-peak`
  - `final-file-visual-pass-audio-gate-fail`
  - `no-master-limiter-or-loudness-normalizer`
- promoted:
  - `none`；音频门槛已有正式契约，本次只新增真实成片反例。
- verification:
  - 媒体参数：H.264、1920×1080、60fps、AAC 48kHz 双声道、896.213 秒；
  - 全片完整解码与 180 个五秒抽帧点；
  - 片头、五个章节卡、四个历史闪帧、字幕 canary 和片尾逐帧接触表；
  - integrated loudness 约 `-20.6 LUFS`、LRA `4.6 LU`、true peak 约 `-0.02 dBFS`；
  - 视觉项通过，音频发布门禁未通过。

## 2026-07-25 · AI Hero 00:29 漏检返修回执
...
- task_tags:
  ...
  - `dual-source-layout`
  - `scene-score-review`
  - `partial-export-timebase`
- cases_read:
  - `FR-2026-07-24-AI-HERO-001`
- adopted:
  - 把用户点名的 `00:29` 视为独立 E2 证据，回到最终文件逐帧定位 `28.933s` 裸帧。
  - 高 scene-score 即使位于预期章节转场，也必须检查命中帧和相邻输出子帧，不允许按“正常转场”直接豁免。
  - 对同一布局边界的 V2 录屏与 V3 人物两路可见来源都增加一帧视觉 guard；保留音频、source time、shader、MG 和 Hyperframes。
  - 先做 30fps 云端边界帧，再做正确秒域的 1080p60 局部导出逐帧验证。
  - 同类问题先扫描结构谓词，再结合章节 / 布局换主和最终像素证据决定是否修，不对普通 clip-anchored 切点全局盲加淡入。
- rejected_or_deferred:
  - 不只修人物层后就停止；第一次局部导出仍发现录屏首个子帧裸露。
  - 不把 236 个 FX 从 local frame 0 开始的普通切点全部加 fade。
  - 不用 `fps=60` 时未经核对的 frame-domain 局部导出做证据；第一次请求实际偏移到约 14 秒。
  - 尚未重新导出完整 1080p60 成片。
  - 本轮只积累 field report，不升级版本、不创建 GitHub Release、不同步 ClawHub。
- new_canaries:
  ...
  - `partial-export-frame-timebase-mismatch`
- promoted:
  - `none`；作为下一次 Skill 迭代候选，等待跨项目复验后统一提升。
- files_changed:
  - `03-实操迭代与踩坑/迭代记录.md`
  - `03-实操迭代与踩坑/案例/2026-07-24-AI-Hero母片精修.md`
- verification:
  - V2 / V3 两路 item 属性回读；
  - frame `868..874` 云端像素接触表；
  - 28.7–29.3 秒 1080p60 局部导出共 36 帧；
  - 边界输出子帧逐帧检查：没有原始全屏人物或未套框架的全屏录屏；
  - V2 / V3 全片 clip-anchored 起点结构扫描：236 个候选，未进行危险的全局批改。
