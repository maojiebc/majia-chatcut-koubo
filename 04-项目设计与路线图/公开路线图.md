...
V1.6.0 已在上述工程底座之上交付面向普通创作者的产品层：一句话意图路由、三种模式、四套默认方案、运行清单、风险决定、按片长自适应的代表样片、审批指纹、跨会话状态、恢复命令和自然语言交付报告。旧 Rule Registry、Creator OS IR/SRT、可恢复执行、媒体检查、反馈治理和可选外挂继续保留为按需底座。

下一阶段的首要目标不是继续扩目录，而是完成真实 ChatCut 证据闸：至少 5 条匿名样本、三种长度、三种内容形态、超时前/后与部分写入、手工修改保护、实际合成画面和人耳试听。达到前，真实端到端保持 `UNVERIFIED`。
...
| 层 | 责任 | 主要公开产物 |
|---|---|---|
| Knowledge | 人与 Agent 可读的方法、事故和操作手册 | `SKILL.md`、`02-剪辑方法手册/` |
| Optional Extensions | 受治理的方法候选和外部镜头配方，不改变硬规则或工具真相 | `extensions/`、checksum、provider allowlist |
...
目标：让普通创作者不理解内部引擎也能安全进入、查看、确认、恢复并接收一个可编辑时间线。

当前交付：

- 稳剪、快剪、专业增强、只审核、继续上次五类意图路由；
- `balanced-stable`、`tight-short`、`trust-longform`、`screen-demo` 四套默认方案；
- Project Brief、Source Inventory、Run Manifest、Decision Log、Checkpoint、Handoff Report 数据合同和匿名模板；
- `run / status / review / approve-decisions / approve-sample / request-revision / resume / report` 主命令；
- 低/中/高风险硬边界、按片长自适应样片、计划/风格/版式/字幕/时间线版本/样片窗口六维审批绑定；
- 阻断原因专属对账、部分写入补偿、手工修改保护、持久化检查点和三轮止损；
- 结构、画面、音频测量、人耳试听、隐私、样片确认和最终确认分项报告；每项证据明示 live 或 simulation provenance；
- 11 个匿名场景、入口路由、文档路由和真实声明资格进入 `npm run verify`。

退出条件：

- 至少 5 条真实匿名 ChatCut canary，覆盖三种长度和三种内容形态；
- timeout-before、timeout-after、partial-write 与用户手工修改保护真实通过；
- 未批准高风险操作、重复写入和手工修改覆盖均为 0；
- 当前 ChatCut build 与工具说明证据未过期；
- 结构、合成画面、音频测量、人耳试听和用户审批不混为同一结论；
- 只有全部满足后，真实报告才允许 `stableClaimEligible=true`。
...
| 12 | One-sentence creator path — OFFLINE SHIPPED | modes/profiles、run manifest、risk/sample/recovery/handoff、CLI |
| 13 | Live ChatCut release gate — NEXT | ≥5 anonymous canaries、current capability、render/listening evidence |
...
- 用户批准范围不会被自动外扩。
- 默认交付停在 `review_ready`，音乐、动态图形、补充画面、生成、导出和发布均未被隐式授权；
- 真实 canary report 与公开能力表述一致，模拟证据不能升级真实状态。
